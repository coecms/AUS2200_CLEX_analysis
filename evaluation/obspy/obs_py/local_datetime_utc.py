...
def local_datetime_utc (local_dt, obs_tz, odt_fmt):
    ...
    obdate = np.array([ int(dt.strftime(dd,odt_fmt.date_fmt())) for dd in tt] )
    obtime = np.array([ int(dt.strftime(dd,odt_fmt.time_fmt())) for dd in tt] )
    return obdate, obtime

class test_local_datetime_utc(unittest.TestCase):
   def test_rtn(self):
      local_dt = [dt.strptime('202101040645','%Y%m%d%H%M'), 
                  dt.strptime('202106050615','%Y%m%d%H%M') ]
      obs_tz = [ZoneInfo('Australia/South')]*2
      odt_fmt = dt_fmt()
      od, ot = local_datetime_utc(local_dt, obs_tz, odt_fmt )
